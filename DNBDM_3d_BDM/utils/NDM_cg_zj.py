...
# ========= 参数 =========
score_dir = Path(r"/hy-tmp/DDMU-1.0_y_16cg")   # NDM/cg 都在这里
out_dir   = Path(r"/hy-tmp/DDMU-1.0_16cg")      # 融合结果输出目录
alpha     = 1.0                            # 融合权重
# ========================
...
for ndm_path in ndm_files:
    ...
    if not cg_path.exists():
        cands = list(score_dir.glob(f"{pid}*-16cg-36size.tif")) + \
    ...
    # 保存
    out_path = out_dir / f"BDM_16cg_{pid}.tif"
